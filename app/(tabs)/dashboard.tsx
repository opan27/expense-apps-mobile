...
export default function Dashboard() {
    ...
    useEffect(() => {
        const loadExtras = async () => {
            try {
                ...
                setRecentIncome((inc.data?.recent || inc.data?.recentRows || []).slice(0, 5));
                setIncomeCategories(inc.data?.donutChart || []);

                setRecentExpense((exp.data?.recent || exp.data?.recentRows || []).slice(0, 5));
    ...
    const totalIncomeAll = incomeCategories.reduce((a, c) => a + safeNumber(c.amount), 0);
    const totalExpenseAll = expenseCategories.reduce((a, c) => a + safeNumber(c.amount), 0);
    ...
    const openCategoryModal = (type: 'income' | 'expense', index: number) => {
        ...
        const sorted = src.slice().sort((a, b) => safeNumber(b.amount) - safeNumber(a.amount));
        const rank = sorted.findIndex((c) => c.category === cat.category) + 1 || src.length;
            ...
            <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 24 }}>
                ...
                <View style={{ flexDirection: 'row', gap: 14, marginBottom: 20 }}>
                    {[
                        ...
                        >
                            <Text style={{ fontSize: 12, color: '#6B7280', marginBottom: 6 }}>{item.title}</Text>
                ...
                >
                    ...
                    {!hasFinancialData ? (
                        <View style={{ alignItems: 'center', paddingVertical: 24 }}>
                            ...
                            <Text style={{ color: '#9CA3AF', fontSize: 12 }}>Belum ada data.</Text>
                        </View>
                    ) : (
                        <>
                            {/* PieChart dari gifted-charts (center PERFECT) */}
                            <View style={{ alignItems: 'center' }}>
                                <PieChart
                                    data={financialOverviewData.map(item => ({
                                        value: item.amount,
                                        color: item.color,
                                    }))}
                                    radius={80}
                        ...
                        >
                            <View>
                                ...
                                <Text style={{ fontSize: 12, color: '#6B7280' }}>
                                    Klik nama kategori di bawah untuk melihat detail.
                                </Text>
                            </View>
                        </View>
                                ...
                                >
                                    <BarChart
                                        ...
                                        data={{
                                            ...
                                            datasets: [
                                                {
                                                    data: incomeCategories.map((c) => safeNumber(c.amount)),
                                                },
                                            ],
                                        }}
                ...
                >
...

                    {
                        extraLoading && !billsSummary ? (
                            <Text style={{ color: '#9CA3AF', fontSize: 12 }}>Memuat ringkasan cicilan...</Text>
                        ) : billsSummary ? (
                            <View>
                                <View
                                    style={{
                                        padding: 10,
                                        borderRadius: 10,
                                        backgroundColor: billsSummary.status === 'warning' ? '#FEE2E2' : '#ECFDF3',
                                        borderWidth: 1,
                                        borderColor: billsSummary.status === 'warning' ? '#FCA5A5' : '#BBF7D0',
                                        marginBottom: 8,
                                    }}
                                >
                                    <Text style={{ fontSize: 12, color: billsSummary.status === 'warning' ? '#B91C1C' : '#166534', marginBottom: 4, fontWeight: '600' }}>
                                        Total cicilan bulan ini: {formatRupiah(billsSummary.totalMinimum)}
                                    </Text>
                                    <Text style={{ fontSize: 12, color: billsSummary.status === 'warning' ? '#B91C1C' : '#166534' }}>
                                        Sisa saldo setelah bayar minimum: {formatRupiah(billsSummary.balanceAfterMinimum)}
                                    </Text>
                                </View>

                                {!!(billsSummary.message || billsSummary.messages) && (
                                    <View style={{ padding: 8, borderRadius: 8, backgroundColor: billsSummary.status === 'warning' ? '#FEE2E2' : '#DCFCE7' }}>
                                        <Text style={{ fontSize: 12, color: billsSummary.status === 'warning' ? '#B91C1C' : '#166534' }}>
                                            {billsSummary.message || billsSummary.messages}
                                        </Text>
                                    </View>
                            ...
                            <Text style={{ color: '#9CA3AF', fontSize: 12 }}>Belum ada ringkasan cicilan.</Text>
                        )
                    }
                </View >
...

                    {
                        upcomingInstallments.length === 0 ? (
                            <View style={{ alignItems: 'center', paddingVertical: 16 }}>
                                <View style={{ width: 48, height: 48, borderRadius: 999, backgroundColor: '#E5E7EB', alignItems: 'center', justifyContent: 'center', marginBottom: 6 }}>
                                    <Ionicons name="layers-outline" size={24} color="#9CA3AF" />
                                </View>
                                <Text style={{ color: '#9CA3AF', fontSize: 12 }}>Tidak ada cicilan jatuh tempo.</Text>
                            </View>
                        ) : (
                            upcomingInstallments.map((item) => (
                                <View key={item.id} style={{ padding: 10, borderRadius: 10, backgroundColor: '#F9FAFB', borderWidth: 1, borderColor: '#E5E7EB', marginBottom: 8 }}>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
                                        <View style={{ flex: 1, paddingRight: 8 }}>
                                            <Text style={{ fontSize: 13, fontWeight: '600', color: '#111827', marginBottom: 2 }}>{item.name}</Text>
                                            <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                                                <Text style={{ fontSize: 11, color: '#9CA3AF', marginRight: 6 }}>Tgl {item.dueDay}</Text>
                                                <Text style={{ fontSize: 11, color: '#6B7280', marginRight: 6 }}>{item.remainingMonths} bln</Text>
                                            </View>
                                        </View>

                                        <View style={{ alignItems: 'flex-end' }}>
                                            <Text style={{ fontSize: 12, fontWeight: '600', color: '#16A34A' }}>{formatRupiah(item.monthlyPayment)}</Text>
                                            <Text style={{ fontSize: 11, color: '#6B7280' }}>per bulan</Text>
                                        </View>
                                    </View>

                                    <TouchableOpacity onPress={() => router.replace(`/expense-add?installment_id=${item.id}`)} style={{ marginTop: 6, alignSelf: 'flex-start', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 999, backgroundColor: '#7C3AED' }}>
                                        <Text style={{ fontSize: 12, color: 'white', fontWeight: '600' }}>Bayar</Text>
                                    </TouchableOpacity>
                                </View>
                            ))
                        )
                ...
                {/* Aktivitas Terkini */}
                < View style={{ backgroundColor: 'white', padding: 16, borderRadius: 14, shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 4, elevation: 3, marginBottom: 20 }}>
                    <Text style={{ fontSize: 16, fontWeight: '700', marginBottom: 12 }}>Aktivitas Terkini</Text>

                    {
                        Array.isArray(summary.latest) && summary.latest.length > 0 ? (
                            summary.latest.map((tx: any, index: number) => (
                                <View key={tx.id ?? index} style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 }}>
                                    <View>
                                        <Text style={{ fontWeight: '600', marginBottom: 2 }} numberOfLines={1}>{tx.category}</Text>
                                        <Text style={{ fontSize: 12, color: '#6B7280' }}>{formatDateId(tx.date)}</Text>
                                    </View>

                                    <Text style={{ fontWeight: '700', color: tx.type === 'income' ? '#16A34A' : '#DC2626' }}>
                                        {tx.type === 'income' ? '+ ' : '- '}
                    ...
                    < View style={{ backgroundColor: 'white', padding: 16, borderRadius: 14, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 4, elevation: 2 }}>
...

                        {
                            recentExpense.length === 0 ? (
                                <Text style={{ color: '#9CA3AF', fontSize: 12 }}>Belum ada pengeluaran.</Text>
                            ) : (
                                recentExpense.map((exp, idx) => (
                                    <View key={exp.id ?? idx} style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                                        <View style={{ flex: 1, paddingRight: 8 }}>
                                            <Text style={{ fontSize: 13, fontWeight: '600', color: '#111827' }} numberOfLines={1}>{exp.category}</Text>
                                            <Text style={{ fontSize: 11, color: '#6B7280' }}>{formatDateId(exp.date)}</Text>
                                        </View>
                                        <Text style={{ fontSize: 13, fontWeight: '700', color: '#DC2626' }}>- {formatRupiah(exp.amount)}</Text>
                                    </View>
                                ))
                            )
                        }
                    ...
                    < View style={{ backgroundColor: 'white', padding: 16, borderRadius: 14, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 4, elevation: 2 }}>
...

                        {
                            recentIncome.length === 0 ? (
                                <Text style={{ color: '#9CA3AF', fontSize: 12 }}>Belum ada pemasukan.</Text>
                            ) : (
                                recentIncome.map((inc, idx) => (
                                    <View key={inc.id ?? idx} style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                                        <View style={{ flex: 1, paddingRight: 8 }}>
                                            <Text style={{ fontSize: 13, fontWeight: '600', color: '#111827' }} numberOfLines={1}>{inc.category}</Text>
                                            <Text style={{ fontSize: 11, color: '#6B7280' }}>{formatDateId(inc.date)}</Text>
                                        </View>
                                        <Text style={{ fontSize: 13, fontWeight: '700', color: '#16A34A' }}>{formatRupiah(inc.amount)}</Text>
                                    </View>
                                ))
                            )
                        }
            ...
            {/* Modal Analisis Kategori */}
            {
                selectedCategory && modalType && (
                    ...
                    >
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                            <View style={{ flex: 1, paddingRight: 8 }}>
                                <Text style={{ fontSize: 16, fontWeight: '700', marginBottom: 2 }}>{selectedCategory.category}</Text>
                                <Text style={{ fontSize: 12, color: '#6B7280' }}>Kategori {modalType === 'income' ? 'Pemasukan' : 'Pengeluaran'}</Text>
                            </View>
                        ...
                        <View style={{ backgroundColor: modalType === 'income' ? '#F5F3FF' : '#FEF2F2', padding: 14, borderRadius: 12, marginBottom: 12 }}>
                            <Text style={{ fontSize: 12, color: '#6B7280', marginBottom: 4 }}>Total Jumlah</Text>
                            <Text style={{ fontSize: 20, fontWeight: '800', color: modalType === 'income' ? '#7C3AED' : '#DC2626' }}>
                                {modalType === 'income' ? '+' : '-'}
                        ...
                        <View style={{ height: 8, borderRadius: 999, backgroundColor: '#E5E7EB', overflow: 'hidden', marginBottom: 6 }}>
                            <View style={{ height: '100%', width: `${selectedCategory.percentage.toFixed(1)}%`, backgroundColor: modalType === 'income' ? '#7C3AED' : '#DC2626' }} />
                        </View>
                        <Text style={{ fontSize: 12, fontWeight: '600', textAlign: 'right' }}>{selectedCategory.percentage.toFixed(1)}%</Text>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                            <TouchableOpacity
                                onPress={() => {
                                    ...
                                    router.replace(modalType === 'income' ? '/(tabs)/income' : '/(tabs)/expense');
                                }}
                                style={{ flex: 1, marginRight: 8, paddingVertical: 10, borderRadius: 999, backgroundColor: modalType === 'income' ? '#7C3AED' : '#DC2626', alignItems: 'center' }}
                            >
                                <Text style={{ color: 'white', fontWeight: '600', fontSize: 13 }}>Lihat Halaman {modalType === 'income' ? 'Pemasukan' : 'Pengeluaran'}</Text>
                            </TouchableOpacity>

                            <TouchableOpacity onPress={() => setSelectedCategory(null)} style={{ paddingVertical: 10, paddingHorizontal: 16, borderRadius: 999, borderWidth: 1, borderColor: '#D1D5DB', alignItems: 'center', justifyContent: 'center' }}>
